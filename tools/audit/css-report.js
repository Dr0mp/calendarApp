...
const imp=[];root.walkDecls(d=>{if(d.important&&!/^\.u-/.test(d.parent.selector||'')&&d.parent.selector!=='[hidden]')imp.push(d.source.start.line+' '+d.parent.selector?.slice(0,50)+' '+d.prop)});P('\n## !important: '+imp.length);imp.forEach(x=>P(' '+x));
